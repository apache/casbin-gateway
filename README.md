...
- **[Connect an application once, for every agent](docs/user-manual/en/2-agents/2.5-connections.md)** — GitHub, Notion, Lark, Figma, a browser and a dozen more. The credential stays in Gateway rather than in each agent's config file, every call through it is checked against that agent's permissions, and testing one gives you a switch per tool.
