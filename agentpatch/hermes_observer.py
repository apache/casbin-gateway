...
def _sensitive_path(file_path: Any) -> bool:
...
def _sanitize_tool_arguments(tool_name: str, arguments: Any) -> Any:
...
def _tool_record(action: str, outcome: str, values: dict[str, Any]) -> dict[str, Any]:
    ...
    if action == "call" and values.get("args") is not None:
        data["arguments"] = _sanitize_tool_arguments(tool_name, values.get("args"))
