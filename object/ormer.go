...
func (a *Ormer) createTable() {
	...
	// The sign-ins saved aside for an agent, so a subscription account and an
	// API key are both held while the agent itself keeps only one.
	err = a.Engine.Sync2(new(AgentAccount))
	if err != nil {
		panic(err)
	}
